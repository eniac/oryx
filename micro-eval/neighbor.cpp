...
int main(int argc, char **argv) {
    ...
    unique_ptr<ServerSecretShares> back_up_local_shares =
        server.prepareSecretSharesForNeighborPass(max_degree, detect_length_k,
                                                  nodes_num, path_num,
    ...
    for (int i = 0; i < run_times; i++) {
        ...
        server.neighborPassWrapper(local_shares);
        auto end = system_clock::now();
        cout
            << "[TIME] round " << i << " neighbor passing takes: "
            << duration_cast<std::chrono::duration<double>>(end - start).count()
            << " seconds, with nodes_num: " << nodes_num
            << ", path_num: " << path_num << ", in total "
            << nodes_num * (1 + path_num) << " tuples" << endl;
    }
