...
int main(int argc, char **argv) {
    ...
    cout << "server init\n";
    server.connectWithOtherServers();

    unique_ptr<ServerSecretShares> back_up_first_s =
        make_unique<ServerSecretShares>();
    unique_ptr<ServerSecretShares> back_up_second_s =
        make_unique<ServerSecretShares>();

    server.prepareSecretSharesForShuffle(max_degree, detect_length_k, path_num,
                                         back_up_first_s, back_up_second_s,
                                         local_compute_parallel_num);

    for (int i = 0; i < run_times; i++) {
        server.assignToSSForShuffle(back_up_first_s, back_up_second_s);

        auto start = system_clock::now();
        server.shuffleWrapper(max_degree, detect_length_k);
        auto end = system_clock::now();
        cout
            << "[TIME] round " << i << " shuffle takes: "
            << duration_cast<std::chrono::duration<double>>(end - start).count()
            << " seconds, with " << path_num << " tuples" << endl;
    }

    server.closeConnectionWithOtherServers();
    return 0;
}