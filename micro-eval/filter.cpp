...
int main(int argc, char **argv) {
    ...
    double valid_ratio = accessDoubleFromConfig(config_map, "valid_ratio");
    double cycle_ratio = accessDoubleFromConfig(config_map, "cycle_ratio");
    ...
    unique_ptr<ServerSecretShares> back_up_local_shares =
        server.prepareSecretSharesForFilter(max_degree, detect_length_k,
                                            path_num, valid_ratio, cycle_ratio,
    ...
    for (int i = 0; i < run_times; i++) {
        ...
        unique_ptr<ServerSecretShares> cycle_shares =
            make_unique<ServerSecretShares>();
        server.filterWrapper(local_shares, cycle_shares);
        auto end = system_clock::now();
        cout
            << "[TIME] round " << i << " filter takes: "
            << duration_cast<std::chrono::duration<double>>(end - start).count()
            << " seconds, with tuples of " << path_num
            << ", valid ratio: " << valid_ratio
            << ", cycle ratio: " << cycle_ratio << endl;
    }
